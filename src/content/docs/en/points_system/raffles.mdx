...
This game has 3 Admin Actions, These 3 actions will handle all the logic. `[Currency Admin] - [Raffle] - Start` this action is how you will start the raffles, In the import its tied to `!raffle` and the way to use it is to do `!raffle <amount>` if you don't provide an amount it will use the default prize, you can also use `k` and `m` for 1000 and 1,000,000. `[Currency Admin] - [Raffle] - Join` is how users will join from the import it is set from `!ticket`. `[Currency Admin] - [Raffle] - End` this action will trigger automattically when the raffle time is up.
