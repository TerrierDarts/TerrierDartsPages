...
First thing you will need to do is in `[Hangman Admin 0] Start New Game`, you will need to edit the OBS Source Visability `Sub-Action` to the one youve created, if youve chosen to not use the overlay you can disable this action, if you are using SLOBS you will need to replace it with the StreamLabs equivalent, then you need to edit the word list to match the file path of your word lists. I've provided a few word lists below if you dont have one.
...
Action Names can be changed but I would advise against it, however if you delete any actions by mistake you will need to reimport the ac
